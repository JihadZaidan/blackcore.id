      ...
      <div class="card">
        <div class="card-body">
          <div class="card-title d-flex align-items-center gap-2">
            ...
            <h4 class=" m-0">Edit Artikel</h4>
          </div>

          <form method="POST" id="editArtikelForm" action="{{ route('admin.articles.update', $article) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
              ...
              <div class="col-12 col-md-6">
                <div class="form-group">
                  ...
                  <input type="text" class="form-control" id="title" name="title" placeholder="Masukkan judul"
                    data-slug-target="#slug-article" value="{{ old('title', $article->title) }}" required>
                  @error('title')
              ...
              <div class="col-12 col-md-6">
                <div class="form-group">
                  ...
                  <input type="text" class="form-control" id="slug-article" name="slug"
                    placeholder="Ketik judul untuk generate slug otomatis" value="{{ old('slug', $article->slug) }}" required>
                  @error('slug')
                    <small class="text-danger">{{ $message }}</small>
                  @enderror
                </div>
                ...
                <div class="form-group">
                  ...
                  <div class="d-flex">
                    <select type="text" class="form-control" id="tags" name="tags[]" multiple
                      ...
                      @foreach ($tags as $tag)
                        <option value="{{ $tag->id }}" {{ in_array($tag->id, $article->tags->pluck('id')->toArray()) ? 'selected' : '' }}>{{ $tag->tag_name }}</option>
                      @endforeach
                    </select>
                    <button type="button" class="btn btn-light btn-sm btn-inside" data-toggle="modal"
            ...
            <textarea id="tinyeditor" name="content" class="mb-4" class="min-height: 500px">{{ $article->content }}</textarea>
            @error('content')
              <small class="text-danger">{{ $message }}</small>
            @enderror
            ...
            <div class="form-group mt-4">
              ...
              <div class="mt-3">
                <img id="uploadCoverPreview" src="{{ $article->cover_image ? asset('storage/' . $article->cover_image) : '' }}" alt="Product Image Preview"
                  style="max-width: 70%;margin: 0 auto; {{ $article->cover_image ? '' : 'display: none;'}}">
              </div>
              @error('cover_image')
                <small class="text-danger">{{ $message }}</small>
              @enderror
            ...
            <button type="button" onclick="tinymce_submit()" class="btn btn-dark">Update Artikel</button>
          </form>
        </div>
      </div>
    </div>
  ...
  @push('scripts')
    <script>
      ...
      function tinymce_submit() {
        let form = document.getElementById('editArtikelForm');
